...
class ConnectivitySchematic(Data):
    def compute(self, *args, **kwargs):
...
class ConnectivityDynamics(Data):
    @staticmethod
...
class FigConnectivitySchematic(Figure):
    @staticmethod
    def plot(args, plot_data):
        ...
        Svals = plot_data.Svals
        details = plot_data.details
        parent_glom = plot_data.parent_glom
        sis_id = plot_data.sis_id
        A1 = plot_data.A1
        A_mean = plot_data.A_mean
        sis_cols = plot_data.sis_cols
        glom_cols = plot_data.glom_cols
...
class FigInferenceDynamics(Figure):
    @staticmethod
...
class FigInferringThePrior(Figure):
    @staticmethod
    def plot(args, plot_data):
...
conn_schematic = ConnectivitySchematic()
conn_dynamics  = ConnectivityDynamics()
inf_dynamics   = InferenceDynamics()
inf_priors     = InferringThePrior()

figure_order = [
    FigConnectivitySchematic(conn_schematic),
    FigInferenceDynamics(inf_dynamics),
    FigConnectivityDynamics(conn_dynamics),
    FigInferringThePrior(inf_priors),
]        
